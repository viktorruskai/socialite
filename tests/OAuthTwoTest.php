...
class OAuthTwoTest extends TestCase
{
    ...
    public function testUserReturnsAUserInstanceForTheAuthenticatedRequest()
    {
        ...
        $response->expects('getBody')->andReturns('{ "access_token" : "access_token", "refresh_token" : "refresh_token", "refresh_token_expires_in" : 31536000 , "expires_in" : 3600 }');
        $user = $provider->user();
        ...
        $this->assertSame('access_token', $user->token);
        $this->assertSame('refresh_token', $user->refreshToken);
        $this->assertSame(31536000, $user->refreshTokenExpiresIn);
        $this->assertSame(3600, $user->expiresIn);
        $this->assertSame($user->id, $provider->user()->id);
    }
